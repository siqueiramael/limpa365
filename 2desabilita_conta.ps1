# ================================================================
# Script 2: Desabilitar Contas em Massa (FASE 1)
# Descricao: Desabilita contas (REVERSIVEL - pode reativar depois)
...
Write-Host "=============================================="
Write-Host "ATENCAO: DESABILITACAO DE CONTAS" -ForegroundColor Yellow
Write-Host "=============================================="
Write-Host "Este script ira DESABILITAR contas."
Write-Host "Usuarios nao poderao fazer login."
Write-Host "Esta acao e REVERSIVEL (pode reativar depois)."
Write-Host "=============================================="
...
$logSucesso = "log_desabilitar_sucesso_$timestamp.csv"
$logErro = "log_desabilitar_erro_$timestamp.csv"
...
for ($i = 0; $i -lt $totalUsuarios; $i += $TamanhoBatch) {
    ...
    foreach ($usuario in $batch) {
        ...
        try {
            ...
            # Verificar se ja esta desabilitado
            if (!$mgUser.AccountEnabled) {
                Write-Host "    [INFO] Usuario ja estava desabilitado" -ForegroundColor Cyan
                
                $sucessos += [PSCustomObject]@{
                    Email = $email
                    Nome = $mgUser.DisplayName
                    Status = "Ja estava desabilitado"
            ...
            if (!$DryRun) {
                ...
                Write-Host "    [OK] Conta desabilitada com sucesso!" -ForegroundColor Green
            } else {
                Write-Host "    [TESTE] Desabilitacao simulada (dry-run)" -ForegroundColor Yellow
            }
            ...
            $sucessos += [PSCustomObject]@{
                Email = $email
                Nome = $mgUser.DisplayName
                Status = "Desabilitado com sucesso"
                Data = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            }
...
if ($DryRun) {
    ...
    Write-Host "`n[IMPORTANTE] Aguarde 7-15 dias antes de executar o Script 3" -ForegroundColor Yellow
    Write-Host "Para reativar uma conta, use:" -ForegroundColor Cyan
    Write-Host "Update-MgUser -UserId <email> -AccountEnabled:`$true" -ForegroundColor White
}
